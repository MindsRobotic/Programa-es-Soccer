...
int toleranciaAlinhamento = 5;
int limiarBolaPerto = 220;

// --- Pesos fixos ---
float pesoIR_perto = 0.3;      // IR quando bola perto
float pesoBussola_longe = 0.5; // Bússola quando bola longe
...
// --- Controle de giro especial ---
bool girando = false;
unsigned long inicioGiro = 0;
int sentidoGiro = 1;
unsigned long tempoGiroMax = 2000;

// --- Heading do nosso gol ---
float headingNossoGol = 0;
...
void setup() {
  ...
  headingNossoGol = headingZero + 180;
  if (headingNossoGol >= 360) headingNossoGol -= 360;
...
void loop() {
  ...
  if (angleZ < -180) angleZ += 360;
  if (abs(angleZ) > 90) angleZ += (angleZ > 0) ? -180 : 180;
  ...
  // Giro especial
  if (!girando && ballIntens >= limiarBolaPerto && abs(erroCompassFiltrado) > 150) {
    girando = true;
    inicioGiro = millis();
    sentidoGiro = (erroCompassFiltrado > 0) ? 1 : -1;
  }

  if (girando) {
    int velGiro = 180 * sentidoGiro;
    // Motores para girar no lugar com ajuda do traseiro
    int velEsq  = -velGiro;
    int velDir  =  velGiro;
    int velTras = -velGiro;
    Deslocar(velEsq, velDir, velTras);

    if (abs(erroCompassFiltrado) < toleranciaAlinhamento || millis() - inicioGiro > tempoGiroMax) {
      girando = false;
    }
    Serial.println("** GIRO ESPECIAL ATIVO COM MOTOR TRASEIRO **");
    return;
  }
